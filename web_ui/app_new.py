"""多模态纠错 Web UI：仅保留ASR纠错功能；默认 Transformers（LlamaFactory ChatModel）；可选 Ollama。"""
...
async def correct(
    ...
    # ========== 仅允许asr模式 ==========
    if mode not in ("asr",):
        raise HTTPException(status_code=400, detail="mode 仅支持 asr")
